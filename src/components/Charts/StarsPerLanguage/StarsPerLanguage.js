...
const sortStarsPerLanguageDescending = (languages) => {
  const sortedStarsPerLanguageDescending = Object.entries(languages).sort(
    ([, a], [, b]) => b - a
  );
...
const formatStarsPerLanguage = (languages) => {
  const formattedStarsPerLanguage = languages.map(([language, stars]) => ({
    language,
    stars,
  }));
...
const StarsPerLanguage = () => {
  ...
  const sortedStarsPerLanguageDescending = sortStarsPerLanguageDescending(
    starsPerLanguage
  );

  const mostStarredLanguages = getSelectionFromArray(
    sortedStarsPerLanguageDescending,
    5
  );

  const formattedStarsPerLanguage = formatStarsPerLanguage(
